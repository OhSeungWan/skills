...
8. **사람에게 가는 것은 여섯.** 도달 실패 · `guessed` · `unpaired` · `anchor_text` 못 찾은 섹션 · `degraded` · **`figma.version`이 바뀐 뒤의 `stale` 섹션**. 마지막 것을 자동으로 걷지 않는다 — 버전이 움직였다는 사실만 신호고 판단은 사람이 한다. 통째 검수는 첫 setup에서 한 번 제안하되 건너뛸 수 있게.
