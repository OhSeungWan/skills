---
name: track
description: 티켓 여러 장으로 진행하는 트랙의 브랜치·PR 라이프사이클. 트랙 착수(통합 브랜치·통합 PR 개설), 티켓 브랜치·티켓 PR 생성, 티켓 PR·통합 PR 머지 직후 손처리가 필요할 때 사용. 사용자가 "트랙 시작하자", "티켓 PR 올려", "머지됐다"고 말할 때, 또는 에이전트가 직접 티켓 PR을 머지했을 때. /track [start|ticket|merged]
---
...
| PR 템플릿 | `.github/pull_request_template.md` 있으면 그 절 구조 안에 대시보드 5블록 배치 |
...
**트랙 대시보드 5블록 — 존재를 강제한다:** ⑴ 티켓 표(번호·상태·머지 SHA) ⑵ **트랙 결정** ⑶ 리뷰 포인트 ⑷ 배포 전 사람 몫 ⑸ 열린 질문. PR 템플릿이 있으면 그 절 구조 안에 배치하고, 블록 내부 양식은 자유.

⑵와 ⑸는 성격이 반대다 — **⑵는 이미 닫힌 결정**(계약 확정·설계 판정)이고 **⑸는 아직 안 닫힌 것**이다. 섞으면 뒤 티켓이 열린 질문을 자기가 닫는다. ⑵가 필요한 이유: 트랙 도중 도착한 계약 변경은 통합 PR 코멘트로 들어오는데, 코멘트는 소진되면 다시 안 읽히므로 **계속 유효한 사실만 ⑵로 승격**해야 뒤 티켓이 본다(무인 러너는 `grind` 참조).

**승격** — 단발로 시작한 PR이 트랙으로 커지면: 통합 브랜치 신설(빈 시작 커밋 포함) → 기존 PR을 `gh pr edit <n> --base <통합>`으로 retarget → 통합 PR 개설.

완료 기준: 통합 PR이 열려 있고 본문에 5블록이 있다.
...
**판정: 머지된 게 티켓 PR인가 통합 PR인가.** 통합 PR이면 절 끝의 「트랙 종료」로 간다.
...
2. 통합 PR 본문 갱신 — 티켓 표에 ✅·머지 SHA 기록, 규모 재실측(`git diff <base tip>..HEAD --stat`).
   이 티켓에서 닫힌 결정이 트랙 전체에 계속 유효하면 「트랙 결정」 절에 한 줄 추가하고, 그 근거가 된 통합 PR 코멘트에 `+1` 리액션으로 소진 표시를 남긴다.
3. 스펙 이슈에 진행 코멘트.
...
### 트랙 종료 — 머지된 게 통합 PR일 때

위 7항목 대신 같은 dry-run → 승인 1회 → 일괄 흐름으로:

1. 스펙 이슈 close — 최종 base가 default 브랜치가 아니면 여기서도 자동 close가 없다. 완료 코멘트 + `gh issue close`.
2. 통합 브랜치 정리 — 원격 자동삭제 여부 확인 후 로컬도 삭제. 남은 티켓 브랜치도 같이.
3. 대시보드 마감 — 「배포 전 사람 몫」·「열린 질문」에 남은 줄을 스펙 이슈나 후속 이슈로 옮긴다. 머지된 PR 본문은 아무도 다시 안 연다.
4. 백로그 미러 종결(관례에 있으면).
5. QA 루프가 걸린 트랙이면(관례 문서가 QA 시트를 지정) `배포 완료`가 아닌 행 수를 보고한다 — 통합 PR이 닫히면 `/qa fix`는 더 안 돈다. 남은 항목은 새 트랙이나 핫픽스 몫이다.

완료 기준: 5항목 각각의 실행 결과 또는 skip 사유가 보고에 있다.
