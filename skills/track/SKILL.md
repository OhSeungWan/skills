...
dry-run 요약에 담을 것: close할 이슈 번호, 통합 PR 본문 변경안, 달 코멘트 문안, **삭제할 원격 브랜치**(스쿼시 자동삭제 설정이면 이미 없음 — 확인해서 표기). 승인 후:
...
3. 스펙 이슈에 진행 코멘트.
4. 백로그 미러 상태 동기화(관례에 있으면).
5. 티켓 브랜치 정리 — 원격 삭제 여부 확인 후 로컬도 삭제.
6. 다음 티켓 차단 해제 코멘트. 다음 티켓이 아직 티켓화 안 됐으면 사용자에게 알린다.

완료 기준: 6항목 각각의 실행 결과 또는 skip 사유가 보고에 있다.
