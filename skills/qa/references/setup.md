...
figma:
  ...
  source: <스펙 이슈 링크>     # 피그마 URL을 캐낸 자리. 없으면 사람에게 묻는다

oracle:                       # 오라클 없는 트랙이면 블록째 생략
  snapshot: <ISO 8601>        # setup이 오라클을 읽은 시각. scan은 오라클을 부르지 않는다
...
screens:
  - slug: <화면 slug>          # 지문 키 첫 조각. 한 번 정하면 바꾸지 않는다
    ...
    flows:                           # 플로우 검사 — 화면 레벨. 캡처(피그마 프레임 수명)와 다른 물건(오라클 항목 수명)이라 캡처에 붙이지 않는다
      - source: <오라클 항목 ID>      # 정답지 출처. 지문 키는 <화면 slug>/<source>, 같은 source 가 한 화면에 여럿이면 둘째부터 -2, -3
...
헤더와 탭바는 QA 대상이다. 화면 `chrome` 한 줄에 섹션 둘, `hide_fixed: false`. 트랙당 한 번이다.
...
7. **한 바퀴 돌려 자동 검증.** 캡처마다 `arrival_text` 도달과 `match_text` 겹침 2개를 확인한다. 대조 시점은 프라이밍 스크롤 뒤 — scan과 같은 시점이어야 한다. 통과한 줄을 `confirmed`로 올린다. **플로우도 이 바퀴에서 돈다** — 통과한 검사만 `confidence: confirmed`. 실패한 검사는 사람이 "검사가 틀림 / 진짜 결함"을 가르고, 진짜면 setup이 그 자리에서 시트 행을 만든다(ADR-0004). scan은 `confirmed` 검사 실패만 행으로 올린다 — 검사 자체가 틀리면 모든 실패가 오탐이라서다.
8. **사람에게 가는 것은 일곱.** 도달 실패 · `guessed` · `unpaired` · `anchor_text` 못 찾은 섹션 · `degraded` · `figma.version`이 바뀐 뒤의 `stale` 섹션 · **오라클 "시안과 다름" 항목의 `stale` 초안**(아래 절). 버전이 바뀐 `stale`을 자동으로 걷지 않는다 — 버전이 움직였다는 사실만 신호고 판단은 사람이 한다. 통째 검수는 첫 setup에서 한 번 제안하되 건너뛸 수 있게.

## 오라클 stale 초안

setup은 오라클을 **여기서 1회** 읽는다 — 좌표는 `config.yaml` `oracle:` 블록, SQL 쿼터에 막히면 view 모드(`/oracle` 스킬과 같은 폴백). 읽은 시각을 대장 머리말 `oracle.snapshot`에 남긴다. scan은 오라클을 부르지 않는다 — 매 실행이 정답지 원본을 부르지 않는 것은 피그마 캐시와 같은 원칙이다.

④ 화면 기대 항목 중 **"시안과 다름"이 명시된 것**을 골라 항목이 가리키는 시안 프레임·노드가 대장의 어느 캡처·섹션에 물려 있는지 맞춘다. 물린 섹션이 있으면 `stale: <항목 ID> — <사유>` **초안**을 제안 목록(위 8번의 일곱째)에 올리고, 항목의 프레임이 대장에 안 쓰였거나 화면 자체가 없으면 매핑 없음으로 리포트에만 남긴다. 1차 scan에서 화면 하나가 통째로 짝짓기 거부된 원인이 정확히 이 부류다 — 시안에만 있는 요소를 실물 결함으로 읽는다.

초안은 자동 기입이 아니다 — `stale`은 사람 값이다(대상 레포 ADR). 제안에는 대안을 붙인다: `stale`은 섹션 판정 전체를 죽이므로 차이가 요소 하나뿐이면 `ignore` 한 줄이 나을 수 있다. 어느 쪽이든 판정 프롬프트에는 주입하지 않는다 — 시야를 넓히면 정밀도가 떨어진다.

## 오라클 플로우 분해

같은 1회 읽기에서 **④ 화면 기대 `확정` 항목**을 원자 검사로 분해해 화면의 `flows:`에 넣는다(스키마는 위). 정답지는 오라클 ④ 확정뿐이다 — 스펙 이슈·티켓 AC·에이전트 추론은 기대값이 아니다([ADR-0004](../../../.agents/adr/0004-flow-checks-derive-from-oracle.md)).
...
- **`② 정책` 층은 직접 분해하지 않는다** — 정책은 ④ 문장이 인용한다. `잠정` 항목은 시트가 아니라 리포트만이므로 1차 분해 대상이 아니다.

`expect`의 카피 문구는 오라클 문장이 명시한 것만 확정으로 쓰고, 구조만 말한 항목("제목 한 줄, 버튼 없음")은 부재 단언(`no_text`)으로 좁힌다 — 실물에서 읽은 카피를 기대로 되먹이면 검사가 항상 통과한다.
