...
호출: `/qa scan [화면 slug...] [--all] [--flow|--visual]`. `collect`는 사람이 눈으로 찾아 타이핑하는 자리고, scan은 그 자리를 기계가 미리 채운다. 기본은 비주얼(시안 대조) + 플로우(오라클 검사) 둘 다 — `--visual`·`--flow`는 한쪽만 돌릴 때다.
...
4. 행선지는 `confidence`가 가른다(ADR-0004):
   - **`confirmed` 검사 실패 → 시트 행.** `성질` = `로직`, `해결 여부` = 진입 상태. `재현순서` = `before`+`steps`를 사람 문장으로, `비고`에 `qa-key: <화면>/<source>`와 근거(몇 번째 expect · 기대 · 실제), 실패 스크린샷 1장 첨부. 오라클 항목 ID를 행 본문에 남긴다 — 검수하는 사람이 정답지 원문을 찾는 열쇠다.
...
플로우 판정에 서브에이전트·판정 프롬프트는 없다 — expect는 기계 비교다. 판정 프롬프트에 오라클을 주입하지 않는 이유와 같은 원칙의 반대면이다: 비주얼 판정은 넓은 시야가 정밀도를 깎지만, 플로우는 좁은 단언이라 모델 판단 자체가 필요 없다.
